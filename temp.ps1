...
function Disable-IconEditorDevelopmentMode {
  ...
  $resetHelper   = Join-PathSegments @($RepoRoot, 'tools', 'icon-editor', 'Reset-IconEditorWorkspace.ps1')
...
function Invoke-LabVIEWRogueSweep {
  ...
  $outputPath = Join-PathSegments @($rogueDir, ("rogue-sweep-{0}.json" -f (Get-Date -Format 'yyyyMMddTHHmmssfff')))
  try {
...
function Initialize-IconEditorDevModeTelemetry {
  ...
  if ($RepoRoot) {
    $agentWaitPath = Join-PathSegments @($RepoRoot, 'tools', 'Agent-Wait.ps1')
