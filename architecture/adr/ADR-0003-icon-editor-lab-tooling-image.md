...
The icon-editor-lab repository now exposes a rich command surface via x-cli (`tools/x-cli-develop`) and wraps LabVIEW-adjacent workflows (vi-compare, vi-analyzer, vipmcli/g-cli, PPL build) behind JSON-driven commands. We also vendor the **LabVIEW CI/CD Seed 2.2.1** implementation under `tools/seed-2.2.1`, which contains VipbJsonTool plus helper shell scripts (`vipb2json`, `json2vipb`, etc.) for maintaining VIPB files in source control.

Today:

- x-cli and the seed tooling are built ad hoc on developer machines or CI runners.
- Ubuntu workflows (`ci-ubuntu-minimal`, local devcontainers) need a consistent way to:
  ...
  - Call the seed VipbJsonTool helpers for VIPB-related automation.
- Codex agents are expected to call x-cli via `tools/codex/Invoke-XCliWorkflow.ps1` / `Invoke-LabVIEWOperation.ps1`, but there is no pre-built container image that:
  - Contains x-cli and the seed binaries.
...
- Build x-cli and seed *inside* the container from the repo source.
...
- Builds and ships the lab’s .NET tooling from this repo (notably x-cli and the seed VipbJsonTool).
- Exposes those tools as first-class CLIs (`x-cli`, `VipbJsonTool`, `vipb2json`, `json2vipb`, etc.) inside the container.
...
1. **Dockerfile (`tools/Dockerfile.icon-editor-lab-tooling-ubuntu`)**
   ...
   - Multi-stage build:
     - Stage `build`:
       ...
       - `dotnet publish tools/x-cli-develop/src/XCli/XCli.csproj -c Release -r linux-x64 -o /out/xcli`.
       - `dotnet publish tools/seed-2.2.1/src/VipbJsonTool/VipbJsonTool.csproj -c Release -r linux-x64 --self-contained true -p:PublishSingleFile=true -o /out/seed`.
     - Stage `runtime`:
       - Base: `mcr.microsoft.com/dotnet/runtime:<matching-version>` (or `aspnet` if needed).
       - Install PowerShell 7 (`pwsh`), git, curl, unzip, and the dotnet SDK (for local builds/tests).
       - `COPY --from=build /out/xcli /opt/xcli`.
       - `COPY --from=build /out/seed /opt/seed`.
       - Copy the seed shell helpers (`tools/seed-2.2.1/bin/*`) and templates so `vipb2json`, `json2vipb`, and related commands function identically to upstream seed.
       - Add shims to `/usr/local/bin`:
         - `x-cli` → runs the published binary under `/opt/xcli`.
         - `VipbJsonTool` plus the helper scripts → forward into `/opt/seed`.
       - Set defaults:
...
4. **Usage pattern and scope**
   ...
   - Supported operations inside the image:
     ...
     - VIPB operations via the bundled seed CLI (as long as they do not require LabVIEW).
     - Validation suites such as:
       - `Invoke-Pester tests/Codex.XCli.Tests.ps1`.
       - `tools/validation/Invoke-AgentValidation.ps1` in emulated mode (now including a seed round-trip test).
...
- **Reproducible tooling environment**  
  Any developer or CI job that uses the GHCR image gets the same versions of x-cli, the seed CLI, PowerShell, and the dotnet runtime.

- **Faster iteration for Codex and CI**  
  Building x-cli and the seed VipbJsonTool inside the image removes per-job build overhead for workflows that only need the CLIs and emulated providers.

- **Clear separation of responsibilities**  
  The tooling image owns:
    - x-cli, the seed CLI, and orchestration scripts.  
...
- **Image maintenance overhead**  
  The Dockerfile must be kept in sync with:
    ...
    - seed project path and target frameworks.
...
- Align this ADR with ADR-0002 (x-cli staged publish) by:
  ...
  - Documenting in `src/tools/README.md` how to invoke x-cli and the seed CLI from inside the tooling image.
