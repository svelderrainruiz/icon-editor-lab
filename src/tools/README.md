...
| vipmcli package replay | `dotnet run … -- vipm-build-vip --request configs/vipm-build-request.json` (now invokes `Invoke-VipmPackageBuildJob.ps1` with `configs/logs/vipm-build-sample.log`) | `Tools: Smoke vipmcli package`, `Tools: Build VIP (real job)` | Same as above |
...
### VIPB-first real build branch

When you need to iterate on a real `.vip` artifact (typically on a dedicated release or feature branch), treat `NI_Icon_editor.vipb` as the canonical input rather than replay logs. The workflow now begins by snapshotting the upstream vendor repo so every run is auditable:

1. Run `tools/vendor/New-LabviewIconEditorInstance.ps1` to clone/copy `labview-icon-editor` into `vendor/_instances/<timestamp>/`. The helper writes `metadata.json` (source path + commit + timestamp) and rewrites `configs/icon-editor-vendor.json` to point at that instance (`${workspaceFolder}/vendor/_instances/<name>`). Each build therefore records exactly which snapshot provided the VIPB/VIs, making incident reviews straightforward. When you just need to ensure a snapshot exists (e.g., when using the compound VS Code tasks continuously), launch **Vendor: Ensure labview-icon-editor snapshot** — it runs the same helper with `-ReuseExisting`, so it reuses the current instance instead of copying a new one each time. When you intentionally need a fresh copy, run **Vendor: Snapshot labview-icon-editor** manually (no reuse switch).
2. Round-trip the VIPB via `src/tools/icon-editor/Invoke-SeedVipb.ps1` (vipb2json → patch → json2vipb) so edits stay portable and the JSON & VIPB stay in sync. Because x-cli and the helpers now resolve the vendor root through `configs/icon-editor-vendor.json`, switching instances or re-vendoring a fork only requires updating the config (the snapshot helper does this for you).
3. If you need to regenerate release notes, call `src/tools/icon-editor/Generate-ReleaseNotes.ps1` before the build; it wraps the vendored `GenerateReleaseNotes.ps1` helper and can be invoked manually or via the new `Tools: Build VIP (real job)` task.
4. Execute `dotnet run ... -- vipm-build-vip --request configs/vipm-build-request.json` (the new `Tools: Build VIP (real job)` task now calls the same script) to invoke `Invoke-VipmPackageBuildJob.ps1`. x-cli runs the guard first (`src/tools/icon-editor/Test-VipbCustomActions.ps1`), which converts the VIPB with seed and fails fast if any `<Custom_Action_VIs>` entry references a forbidden or missing VI under `.github/actions/build-vi-package`. After the guard passes, the helper reads `configs/logs/vipm-build-sample.log` (default if `LogPath` omitted), regenerates release notes, patches the VIPB, and runs the g-cli/vipm provider to emit the `.vip`. If you want the PPL build + VIP package in one go, use the compound VS Code task **Tools: Build PPL+VIP (real)** which sequences the `Tools: PPL Build (x-cli)` task before the VIP build. When you’re ready to publish x-cli as part of the release, run **Tools: Build & Publish VIP release** to build the PPLs, package the VIP, and then build/package/upload the guarded x-cli binary in one pass.
5. The guard means LabVIEW never starts with a broken VIPB: any missing custom-action VI stops the job immediately, now with per-tag error messages like `VIPB custom action guard failed: - Pre-Install_VI references missing file .\.github\actions\build-vi-package\VIP_Pre-Install Custom Action.vi`. When the provider completes, telemetry about the build is emitted under `tests/results/_agent/icon-editor/vipm-cli-build`. When the provider can’t produce a `.vip`, the script now throws, making failures easier to triage before merging to main.

Keep the validation plan (main branch) on `configs/vipm-build-request.sample.json`, which continues to skip the provider so Docker/CI runs stay NI-free. The real-build branch is where `Invoke-VipmPackageBuildJob.ps1` (and therefore the x-cli `vipm-build-vip` command) runs against an actual toolchain.
