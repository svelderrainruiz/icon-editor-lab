...
## Description
- Accepts a repo path (defaults to `vendor/labview-icon-editor` under the current repo), verifies `BaseRef`/`HeadRef`, and inspects up to `-MaxCommits` (default 50).
...
| --- | --- | --- | --- |
| `RepoPath` | string | `vendor/labview-icon-editor` (relative) | Repository containing VIs to inspect. |
