...
- **Memory**: ≥32 GiB RAM (64 GiB preferred) so LabVIEW 2021 + LabVIEW 2025 preview + vipmcli (g-cli) can co-exist with pwsh/Node/Python tooling without paging.
...
- **vipmcli / g-cli**: Install the JKI VIPM distribution only to obtain the `vipmcli`/`g-cli` binaries (2023.1 or newer). Sign in once so `.vipc` imports succeed, then ensure automation calls the CLI entry points (`g-cli`, `vipmcli`) rather than launching the desktop app.
...
| 45-vipmcli | Optional vipmcli dependency repair stage (`EnableVipmStage`) that runs `local-ci/windows/scripts/Repair-LVEnv.ps1` in display or apply mode. | Ensures `.vipc` dependencies are applied locally via the g-cli provider before analyzer/LUnit stages. |
...
Windows LabVIEW/vipmcli stages honor the following profile settings (see `local-ci/windows/profile.psd1`):
...
- `EnableVipmStage`, `VipmVipcPath`, `VipmRelativePath`, `VipmDisplayOnly` control the vipmcli stage (and can be overridden per run via `LOCALCI_VIPM_VIPC`).
